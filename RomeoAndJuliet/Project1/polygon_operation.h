...
int all_left_or_right(vector<int> chain, int from, int to)
{
	bool neither = false;
	bool left;
	bool pending = true;
	...
	while (pending && i<chain.size())
	{
		if (!is_right(chain[i], from, to))//strictly left
		{
			left = true;
			pending = false;
		}
		else if (!is_left(chain[i], from, to))//strictly right
		{
			left = false;
			pending = false;
		}
	...
	if (pending == true)//neither strictly right nor strictly left
	{
		return 4;
	}
	...
	//see if the rest fits
	for (i; i < chain.size(); i++)
	{
		if (!side(chain[i], from, to))
		{
			return 3; //neither all_right nor all_left
		}
	}
